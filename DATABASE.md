...
| `strike_limit_per_week`  | int | Макс. штрафов за неделю; при превышении — понижение на 1 уровень (null = не понижаем) |
...
| `shifts_completed`| int     | Количество завершённых смен (по умолчанию 0). **При смене уровня** (повышение, понижение по штрафам, ручное в админке) **обнуляется**. |
| `created_at`, `updated_at`, `deleted_at` | timestamp | Общие метки времени |
...
Штрафы за no-show, позднюю отмену и т.п.; используются для логики уровней (лимиты по неделе/месяцу).
...
| `removed_at`       | timestamp | Когда штраф снят (не учитывается в лимитах) |
| `removal_reason`   | varchar(512) | Причина снятия |

**Связи:** многие к одному: `user_id` → `users.id`. Подсчёт штрафов за неделю/месяц идёт по `occurred_at` с учётом `removed_at IS NULL`.
