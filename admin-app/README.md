...
- **Настройки** (`/settings`): бонусы за смены, порог бонусов за месяц для квестов, предрегистрация лояльности, рейтинг надёжности, **автоквест при падении рейтинга** (форма условия квеста без JSON — см. `components/quest-condition-config-fields.tsx`).
