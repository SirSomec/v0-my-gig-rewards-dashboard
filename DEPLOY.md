...
Запускаются **три контейнера**: фронт (app), бэкенд API (api), БД PostgreSQL (db). Каждый сервис в отдельном контейнере; БД разворачивается в контейнере **db** (образ postgres:16-alpine), данные хранятся в Docker volume `pgdata`. При старте API автоматически применяются миграции БД.
