...
1. **Установите зависимости** — при `npm install` автоматически создаётся `.env` из `.env.example` **в корне проекта**.
2. **При необходимости отредактируйте `.env` в корне** — задайте `NEXT_PUBLIC_REWARDS_API_URL` (по умолчанию `http://localhost:3001`) и `NEXT_PUBLIC_DEV_USER_ID` (например `1`). Все переменные для приложения задаются в этом единственном файле (см. раздел 6 про конфигурацию).
...
Запускаются **четыре контейнера**: фронт (app), админ-панель (admin, порт 3002), бэкенд API (api), БД PostgreSQL (db). Каждый сервис в отдельном контейнере; БД разворачивается в контейнере **db** (образ postgres:16-alpine), данные хранятся в Docker volume `pgdata`.

**Миграции БД применяются автоматически.** При каждом запуске контейнера API в entrypoint (`scripts/docker-entrypoint.sh`) перед стартом приложения выполняется `node dist/infra/db/drizzle/migrate.js` — применяются все неприменённые миграции из каталога `nestjs-service/migrations`. Дополнительные действия не требуются: после обновления кода достаточно выполнить `docker compose build` и `docker compose up -d` — новые миграции применятся при старте контейнера API.

*Если после обновления API возвращает 500*, проверьте логи: `docker compose logs api`. Если в логах ошибка вида «column … does not exist», значит миграции не выполнились (например, образ был собран без новых файлов миграций). В этом случае пересоберите образ без кэша (`docker compose build api --no-cache`) и снова запустите контейнеры (`docker compose up -d`). При необходимости миграции можно применить вручную: `docker compose exec api node dist/infra/db/drizzle/migrate.js`, затем `docker compose restart api`.

**Учётные данные БД по умолчанию** (заданы в `docker-compose.yml`; при необходимости переопределяются через `.env`):

| Переменная           | По умолчанию | Описание                    |
|----------------------|--------------|-----------------------------|
| `POSTGRES_USER`      | `rewards`    | Пользователь PostgreSQL     |
| `POSTGRES_PASSWORD`  | `rewards`    | Пароль пользователя         |
| `POSTGRES_DB`        | `rewards`    | Имя базы данных             |
| `DATABASE_URL` (API) | `postgres://rewards:rewards@db:5432/rewards` | Строка подключения для API |

**Требование: один файл `.env` в корне проекта.** Все переменные окружения для всех сервисов (фронт — app, админ-панель — admin, бэкенд API — api, БД — db) задаются в **единственном** файле `.env`, расположенном в корне репозитория рядом с `docker-compose.yml`. Проект рассчитан на запуск в контейнерах: контейнеры получают переменные из этого файла через `env_file: .env` и при необходимости через явные `environment` в `docker-compose.yml`. Не создавайте отдельные `.env` в подпапках (например, в `nestjs-service/` или в `admin-app/`) для деплоя — используйте корневой `.env`. Шаблон переменных: см. `.env.example` в корне проекта.

Свои пароли можно задать в `.env` в корне проекта, например: `POSTGRES_PASSWORD=ваш_надёжный_пароль`. Тогда в `.env` же задайте `DATABASE_URL=postgres://rewards:ваш_надёжный_пароль@db:5432/rewards`, чтобы API подключался с тем же паролем.
...
Для входа через MyGig (по номеру телефона и коду из СМС) дополнительно задайте в `.env`: `NEXT_PUBLIC_MYGIG_API_URL` (URL API MyGig, например `https://main.test09.mygig.ru`) и `REWARDS_INTERNAL_SECRET` (один и тот же секрет для app и api; используется при вызове `POST /v1/auth/ensure-user` для синхронизации пользователя при первом входе).
...
```bash
docker compose ps
# Должны быть контейнеры app, admin, api, db в состоянии running
...
При старте контейнера API автоматически применяются все неприменённые миграции БД (см. раздел 6). Дополнительные команды вручную не нужны.

По умолчанию в одном `docker compose up -d` поднимаются **фронт (app)**, **админ-панель (admin, порт 3002)**, API и БД (см. раздел 6). Если нужен **только фронт** без бэкенда (API уже где-то снаружи), в `docker-compose.yml` можно закомментировать сервисы `api` и `db` и задать в `.env` переменную `NEXT_PUBLIC_REWARDS_API_URL` на внешний URL API.
