...
**Миграции БД применяются автоматически.** При каждом запуске контейнера API (в том числе после `docker compose up -d` или перезапуска) перед стартом приложения выполняются все неприменённые миграции из каталога `nestjs-service/migrations`. Дополнительные действия не требуются: после обновления кода достаточно выполнить `docker compose build` и `docker compose up -d` — новые миграции применятся при старте API.
